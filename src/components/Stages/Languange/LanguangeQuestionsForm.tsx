  ...
  return (
    ...
    >
      ...
      {chooseLang === false ?
        <>
          <h3 className="pl-2">
            Əlavə xarici dil biliklərinizi qeyd edin
          </h3>
          <div className=" flex items-center">
            <button className='add py-2 px-4 w-full h-12 rounded-2xl flex justify-evenly items-center' type="button" onClick={() => setChooseLang(true)}> Əlavə et + </button>
        ...
        :
        isAdding ?
          <LanguageAdd data={questions} addLang={handleAdd} setChooseLang={setChooseLang} /> :
          isEditing.edit ?
            <LanguageAdd data={questions} addLang={handleAdd} editData={isEditing?.data} editLang={editLang} setChooseLang={setChooseLang} /> :
            <>
              <h3 className="pl-2">
                Əlavə xarici dil biliklərinizi qeyd edin
              </h3>
              <button className='add py-2 px-4 w-full h-12 rounded-2xl flex justify-evenly items-center' type="button" onClick={() => setIsAdding(true)}> Əlavə et + </button>
              ...
              <ul>
                {formData?.languageSkills.map((lang: AddLangFormValues, index: number) => (
                  <li key={index} className='border flex-grow rounded-full flex justify-between items-center m-2 relative min-h-[46px] background'>
                    ...
                    <div className="border-r">
                      <div className="level p-2.5">
                        {(lang.ieltsResult?.answer === "4.0" || lang.ieltsResult?.answer === "4.5-5.0" ||lang.toeflResult?.answer==="32-45") ? <span> B1 </span> :
                         (lang.ieltsResult?.answer === "5.5" || lang.ieltsResult?.answer === "6.0" || lang.ieltsResult?.answer === "6.5" || lang.toeflResult?.answer === "46-59" || lang.toeflResult?.answer === "60-78" || lang.toeflResult?.answer === "70-93") ? <span> B2 </span> :
                         (lang.ieltsResult?.answer === "7.0-7.5" || lang.toeflResult?.answer === "94-109") ? <span> C1 </span> :
                         (lang.ieltsResult?.answer === "8.0-9.0" || lang.toeflResult?.answer === "110-120") ? <span> C2 </span> :
                         (lang.toeflResult?.answer === "31") ? <span> A2 </span> :
                    ...
                    <div className="w-48">
                      <div className="certificate py-2.5 px-1.5">
                        {(lang.langCertName && lang.langCertResult) && <>  <span>{lang.langCertName}</span>  <span> {lang.langCertResult}</span> </>}
                        {lang.ieltsResult?.answer && <p className="w-48"> IELTS {lang.ieltsResult.answer}</p>}
                        {lang.toeflResult?.answer && <p className="w-48"> TOEFL {lang.toeflResult.answer}</p>}
                        {(lang.langCert === "1" || lang.engLangCert === "2" ) && <span>Sertifikat yoxdur </span>}
                      </div>
                    </div>
                    <div className="rounded-r-full">
                      <div className="settings flex justify-around p-2.5 gap-2 w-20 h-full items-center">
                        ...
                        <div className="remove cursor-pointer">
                          <img src={removeIcon} alt="remove" onClick={() => { formData?.languageSkills.length === 1 && setIsAdding(true), handleRemove(index) }} />
                        </div>
                      </div>
                    </div>
                  </li>
